...
def listen_for_rc_channel():
    while True:
        ...
        if channel_15_value > 1500:  # assuming 1500 is the threshold value when the button is pressed
            ...
            run_program_after_delay(frame_center_x, frame_center_y)

        time.sleep(0.1)  # Poll every 100ms (adjust based on desired polling rate)
rc_thread = threading.Thread(target=listen_for_rc_channel)
rc_thread.start()
