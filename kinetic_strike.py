...
async def main():
    if len(sys.argv) != 3:
        print("Usage: python localization.py <lat> <lon>")
        sys.exit(1)
...
async def run():
    ...
    mission_items = [
        ...
        #MissionItem(lat, lon, 0, 30, True, float('nan'), float('nan'),
                   # MissionItem.CameraAction.NONE, float('nan'), float('nan'), float('nan'), float('nan'),
                    #float('nan'), MissionItem.VehicleAction.NONE),
    ...
    distance_task = asyncio.ensure_future(print_distance_to_target(drone, lat, lon))
...
async def print_distance_to_target(drone, lat, lon):
    """
